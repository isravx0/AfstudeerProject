...
router.get('/user-info', verifyToken, (req, res) => {
  ...
  db.query('SELECT id, name, email, phoneNumber, location, bio, gender, dob, profilePicture, notifications, mfa_enabled, mfa_secret, mfa_method, mfa_code, mfa_expiry   FROM users WHERE id = ?', [userId], (err, results) => {
    if (err) {
